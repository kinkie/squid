...
// returns: number of entries, or negative to warn of errors
{
    ...
    // try to open file
    if (debug)
        fprintf(debug, "# trying to open %s\n", fn ? fn : "(null)");
    ...
    if (debug)
        fprintf(debug, "# trying to compile \"%s\"\n", expression);
    regex_t rexp;
    ...
    while (cfgin.getline(line, sizeof(line))) {
        ...
        if ((s = (char *)memchr(line, '#', sizeof(line))))
            *s = '\0';
        ...
        if ((result = regexec(&rexp, line, 7, subs, 0)) != 0) {
            ...
            if (debug)
                fprintf(debug, "# match from %d-%d on line %s",
            ...
            if (subs[6].rm_so == -1) {
                ...
                if (debug)
                    fprintf(debug, "# match from %d-%d on \"%s\"\n",

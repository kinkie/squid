...
//          the assertion function will try to create it.
{
    ...
    if (status == 0 && !S_ISDIR(st.st_mode)) {
        ...
        return -1;
    } else if (S_ISDIR(st.st_mode) && !((st.st_uid == geteuid() && ((st.st_mode & S_IWUSR) > 0)) || (st.st_gid == getegid() && ((st.st_mode & S_IWGRP) > 0)) || ((st.st_mode & S_IWOTH) > 0))) {
...
// returns: true, if successful, false otherwise.
{
    ...
    while ((rsize = read(input, buffer, sizeof(buffer))) > 0) {
        if ((wsize = write(out, buffer, rsize)) <= 0)
            break;
    }
    if (rsize < 0 || wsize < 0)
        perror("while copying");

...
{
    return os << "ipcIo" << sio.worker << '.' << sio.msg.requestId << (sio.msg.command == IpcIo::cmdRead ? 'r' : 'w') << sio.disker;
}
...
IpcIoFile::open(int flags, mode_t mode, RefCount<IORequestor> callback)
{
    ...
    if (IamDiskProcess()) {
        ...
        const bool inserted = IpcIoFiles.insert(std::make_pair(diskId, this)).second;
        Must(inserted);
...
{
    debugs(79, 3, HERE << "(disker" << diskId << ", " << readRequest->len << ", " << readRequest->offset << ")");
                         ...
                         IpcIoMsg *const response)
{
    ...
    if (!response) {
        ...
        if (response->xerrno) {
            debugs(79, DBG_IMPORTANT, "ERROR: " << dbName << " read: " << xstrerr(response->xerrno));
            ioError = error_ = true;
...
{
    debugs(79, 3, HERE << "(disker" << diskId << ", " << writeRequest->len << ", " << writeRequest->offset << ")");
                          ...
                          const IpcIoMsg *const response)
{
    ...
    if (!response) {
        ...
        debugs(79, DBG_IMPORTANT, "ERROR: disker " << diskId << " error writing " << writeRequest->len << " bytes at " << writeRequest->offset << ": " << xstrerr(response->xerrno) << "; this worker will stop using " << dbName);
        ioError = error_ = true;
    } else if (response->len != writeRequest->len) {
        debugs(79, DBG_IMPORTANT, "ERROR: disker " << diskId << " wrote " << response->len << " instead of " << writeRequest->len << " bytes (offset " << writeRequest->offset << "); "
    ...
    if (!ioError) {
        debugs(79, 5, HERE << "wrote " << writeRequest->len << " to disker" << diskId << " at " << writeRequest->offset);
    }
...
{
    const std::pair<RequestMap::iterator, bool> result = newerRequests->insert(std::make_pair(id, pending));
...
IpcIoFile::push(IpcIoPendingRequest *const pending)
{
    ...
    try {
        ...
        debugs(47, DBG_IMPORTANT, "ERROR: worker I/O push queue for " << dbName << " overflow: " << SipcIo(KidIdentifier, ipcIo, diskId));  // TODO: report queue len
...
IpcIoFile::canWait() const
{
    ...
    if (expectedWait < 0 || static_cast<time_msec_t>(expectedWait) < config.ioTimeout)
        return true;  // expected wait time is acceptable

    debugs(47, 2, HERE << "cannot wait: " << expectedWait << " oldest: " << SipcIo(KidIdentifier, oldestIo, diskId));
...
IpcIoFile::HandleOpenResponse(const Ipc::StrandSearchResponse &response)
{
    ...
    debugs(47, 4, HERE << "LATE disker response to open for " << response.strand.tag);
...
IpcIoFile::OpenTimeout(void *const param)
{
    ...
    const IpcIoFile *const ipcIoFile = reinterpret_cast<const IpcIoFile *>(param);
    for (IpcIoFileList::iterator i = WaitingForOpen.begin();
         i != WaitingForOpen.end(); ++i) {
        if (*i == ipcIoFile) {
...
IpcIoFile::checkTimeouts()
{
    ...
    if (timeoutsBefore > timeoutsNow) {  // some requests were rescued
        ...
        debugs(47, DBG_IMPORTANT, "WARNING: communication with " << dbName << " may be too slow or disrupted for about " << Timeout << "s; rescued " << (timeoutsBefore - timeoutsNow) << " out of " << timeoutsBefore << " I/Os");
    }

    if (timeoutsNow) {
        debugs(47, DBG_IMPORTANT, "WARNING: abandoning " << timeoutsNow << ' ' << dbName << " I/Os after at least " << Timeout << "s timeout");
    }
...
diskerRead(IpcIoMsg &ipcIo)
{
    ...
    if (read >= 0) {
        ...
        debugs(47, 8, HERE << "disker" << KidIdentifier << " read " << (len == ipcIo.len ? "all " : "just ") << read);
        ipcIo.len = len;
    } else {
        ipcIo.xerrno = errno;
        ipcIo.len = 0;
        debugs(47, 5, HERE << "disker" << KidIdentifier << " read error: " << ipcIo.xerrno);
    }
}
...
diskerWriteAttempts(IpcIoMsg &ipcIo)
{
    ...
    for (int attempts = 1; attempts <= attemptLimit; ++attempts) {
        ...
        if (result < 0) {
            ...
            debugs(47, DBG_IMPORTANT, "ERROR: " << DbName << " failure"
                                                << " writing " << toWrite << '/' << ipcIo.len << " at " << ipcIo.offset << '+' << wroteSoFar << " on " << attempts << " try: " << xstrerr(ipcIo.xerrno));
        ...
        debugs(47, 3, "disker" << KidIdentifier << " wrote " << (wroteNow >= toWrite ? "all " : "just ") << wroteNow << " out of " << toWrite << '/' << ipcIo.len << " at " << ipcIo.offset << '+' << wroteSoFar << " on " << attempts << " try");
    ...
    debugs(47, DBG_IMPORTANT, "ERROR: " << DbName << " exhausted all " << attemptLimit << " attempts while writing " << toWrite << '/' << ipcIo.len << " at " << ipcIo.offset << '+' << wroteSoFar);
...
{
    debugs(47, 7, HERE << "resuming handling requests after " << static_cast<const char *>(source));
...
IpcIoFile::WaitBeforePop()
{
    ...
    if (ipcIo.command == IpcIo::cmdWrite && balance > maxImbalance) {
        ...
        debugs(47, 3, HERE << "rate limiting by " << toSpend << " ms to get" << (1e3 * maxRate) << "/sec rate");
...
IpcIoFile::DiskerHandleRequests()
{
    ...
    while (!WaitBeforePop() && queue->pop(workerId, ipcIo)) {
        ...
        if (elapsedMsec > maxSpentMsec || elapsedMsec < 0) {
            ...
            debugs(47, 3, HERE << "pausing after " << popped << " I/Os in " << elapsedMsec << "ms; " << (elapsedMsec / popped) << "ms per I/O");
            break;
        }
    }
...
IpcIoFile::DiskerHandleRequest(const int workerId, IpcIoMsg &ipcIo)
{
    if (ipcIo.command != IpcIo::cmdRead && ipcIo.command != IpcIo::cmdWrite) {
        debugs(0, DBG_CRITICAL, "ERROR: " << DbName << " should not receive " << ipcIo.command << " ipcIo" << workerId << '.' << ipcIo.requestId);
        return;
    }

    debugs(47, 5, HERE << "disker" << KidIdentifier << (ipcIo.command == IpcIo::cmdRead ? " reads " : " writes ") << ipcIo.len << " at " << ipcIo.offset << " ipcIo" << workerId << '.' << ipcIo.requestId);
    ...
    try {
        ...
        debugs(47, DBG_IMPORTANT, "BUG: Worker I/O pop queue for " << DbName << " overflow: " << SipcIo(workerId, ipcIo, KidIdentifier));  // TODO: report queue len
...
DiskerOpen(const SBuf &path, int flags, mode_t)
{
    ...
    if (TheFile < 0) {
        ...
        debugs(47, DBG_CRITICAL, "ERROR: cannot open " << DbName << ": " << xstrerr(xerrno));
        return false;
    }

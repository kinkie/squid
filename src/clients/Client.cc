...
#if USE_ADAPTATION
        doneWithAdaptation() && Adaptation::Initiator::doneAll() && BodyProducer::doneAll() &&
#endif
...
Client::startRequestBodyFlow()
{
    ...
    if (requestBodySource->setConsumerIfNotLate(this)) {
        debugs(11, 3, HERE << "expecting request body from " << requestBodySource->status());
        return true;
    }

    debugs(11, 3, HERE << "aborting on partially consumed request body: " << requestBodySource->status());
...
Client::startAdaptation(const Adaptation::ServiceGroupPointer &group, HttpRequest *cause)
{
    ...
    if (vrep->expectingBody(cause->method, size) && size) {
        ...
        debugs(93, 6, HERE << "will send virgin reply body to " << virginBodyDestination << "; size: " << size);
        if (size > 0)
...
{
    return !adaptationAccessCheckPending && !virginBodyDestination && !adaptedHeadSource && !adaptedBodySource;
}
...
Client::handleMoreAdaptedBodyAvailable()
{
    ...
    if (spaceAvailable < contentSize) {
        debugs(11, 5, HERE << "postponing storage of " << (contentSize - spaceAvailable) << " body bytes");
...
{
    debugs(11, 5, HERE << "handleAdaptationAborted; bypassable: " << bypassable << ", entry empty: " << entry->isEmpty());
...
Client::handleAdaptationBlocked(const Adaptation::Answer &answer)
{
    ...
    err_type page_id = aclGetDenyInfoPage(&Config.denyInfoList, answer.ruleId.termedBuf(), 1);
    if (page_id == ERR_NONE)
...
Client::calcBufferSpaceToReserve(size_t space, const size_t wantSpace) const
{
    ...
    if (virginBodyDestination != NULL) {
        ...
        debugs(11, 9, "Client may read up to min(" << adaptor_space << ", " << space << ") bytes");
...
Client::replyBodySpace(const MemBuf &readBuf, const size_t minSpace) const
{
    ...
    if (virginBodyDestination != NULL) {
         ...
         */
        size_t adaptation_space = virginBodyDestination->buf().potentialSpaceSize();

        debugs(11, 9, "Client may read up to min(" << adaptation_space << ", " << space << ") bytes");

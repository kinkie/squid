...
{
    if (authSchemes)
        dump_SBufList(entry, ToTree(authSchemes).treeDump(SBuf(name), [](const Acl::Answer &action) {
...
{
    if (ftp_epsv)
        dump_SBufList(entry, ToTree(ftp_epsv).treeDump(SBuf(name), Acl::AllowOrDeny));
}
...
dump_on_unsupported_protocol(StoreEntry *entry, const char *name, acl_access *access)
{
    ...
    if (access) {
        const auto lines = ToTree(access).treeDump(SBuf(name), [](const Acl::Answer &action) {
...
dump_http_upgrade_request_protocols(StoreEntry *entry, const char *rawName, HttpUpgradeProtocolAccess *protoGuards)
{
    ...
    protoGuards->forEach([entry,&name](const SBuf &proto, const acl_access *acls) {
        ...
        const auto acld = ToTree(acls).treeDump(SBuf(""), &Acl::AllowOrDeny);

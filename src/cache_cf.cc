...
configDoConfigure(void)
{
    ...
    if (Config.cacheSwap.swapDirs == nullptr) {
...
parse_refreshpattern(RefreshPattern ** head)
{
    ...
    if ((token = ConfigParser::RegexPattern()) != nullptr) {
...
parse_access_log(CustomLog ** logs)
{
    ...
    if (!token) { // style #1
        ...
        } while ((token = ConfigParser::PeekAtToken()) != nullptr);
    }
...
static void parse_sslproxy_cert_adapt(sslproxy_cert_adapt **cert_adapt)
{
    ...
    if ((al = ConfigParser::NextToken()) == nullptr) {
        xfree(ca);
        self_destruct();
        return;
    }
...
static void dump_sslproxy_cert_adapt(StoreEntry *entry, const char *name, sslproxy_cert_adapt *cert_adapt)
{
    for (sslproxy_cert_adapt *ca = cert_adapt; ca != nullptr; ca = ca->next) {
        storeAppendPrintf(entry, "%s ", name);
...
static void dump_sslproxy_cert_sign(StoreEntry *entry, const char *name, sslproxy_cert_sign *cert_sign)
{
    ...
    for (cs = cert_sign; cs != nullptr; cs = cs->next) {
        storeAppendPrintf(entry, "%s ", name);

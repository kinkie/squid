...
template <class Cbc>
inline std::ostream &
operator<<(std::ostream &os, const CbcPointer<Cbc> &p)
{
    return p.print(os);
}
...
template <class Cbc>
CbcPointer<Cbc>::CbcPointer(const CbcPointer &d) :
    cbc(d.cbc), lock(NULL)
{
...
template <class Cbc>
CbcPointer<Cbc>::CbcPointer(CbcPointer &&d) :
    cbc(d.cbc), lock(d.lock)
{
    d.cbc = nullptr;
    d.lock = nullptr;
}
...
template <class Cbc>
CbcPointer<Cbc> &
CbcPointer<Cbc>::operator=(const CbcPointer &d)
{
...
template <class Cbc>
CbcPointer<Cbc> &
CbcPointer<Cbc>::operator=(CbcPointer &&d)
{
    if (this != &d) {  // assignment to self
        clear();
        cbc = d.cbc;
...
template <class Cbc>
std::ostream &
CbcPointer<Cbc>::print(std::ostream &os) const
{
    return os << cbc << '/' << lock;
}

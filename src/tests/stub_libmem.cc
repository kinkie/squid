...
{
    return memAllocBuf(net_size, gross_size);
}
...
int memInUse(mem_type) STUB_RETVAL(0) void memDataInit(mem_type, const char *, size_t, int, bool) STUB_NOP
...
void MemPools::setIdleLimit(ssize_t) STUB
    ssize_t MemPools::idleLimit() const STUB_RETVAL(0) void MemPools::clean(time_t) STUB
    void MemPools::setDefaultPoolChunking(bool const &) STUB

    //MemAllocator::MemAllocator(char const *aLabel);
    char const *MemAllocator::objectType() const STUB_RETVAL(nullptr) int MemAllocator::inUseCount() STUB_RETVAL(0)
    ...
    MemPoolMeter const &MemImplementingAllocator::getMeter() const STUB_RETSTATREF(MemPoolMeter)
        MemPoolMeter &MemImplementingAllocator::getMeter() STUB_RETSTATREF(MemPoolMeter) void MemImplementingAllocator::flushMetersFull() STUB
    void MemImplementingAllocator::flushMeters() STUB
    void *MemImplementingAllocator::alloc() STUB_RETVAL(nullptr) void MemImplementingAllocator::freeOne(void *) STUB

    MemPoolIterator *memPoolIterate(void) STUB_RETVAL(nullptr)
        MemImplementingAllocator *memPoolIterateNext(MemPoolIterator *) STUB_RETVAL(nullptr) void memPoolIterateDone(MemPoolIterator **) STUB
    int memPoolGetGlobalStats(MemPoolGlobalStats *) STUB_RETVAL(0) int memPoolsTotalAllocated(void) STUB_RETVAL(0)

...
icp_common_t::icp_common_t() STUB
    ...
    bool ICPState::confirmAndPrepHit(const StoreEntry &) STUB_RETVAL(false)
        LogTags *ICPState::loggingTags() STUB_RETVAL(nullptr) void ICPState::fillChecklist(ACLFilledChecklist &) const STUB
...
HttpRequest *
icpGetRequest(char *url, int reqnum, int fd, Ip::Address &from) STUB_RETVAL(NULL) bool icpAccessAllowed(Ip::Address &from, HttpRequest *icp_request) STUB_RETVAL(false) void icpCreateAndSend(icp_opcode, int flags, char const *url, int reqnum, int pad, int fd, const Ip::Address &from, AccessLogEntryPointer) STUB
    icp_opcode icpGetCommonOpcode() STUB_RETVAL(ICP_INVALID) void icpDenyAccess(Ip::Address &from, char *url, int reqnum, int fd) STUB

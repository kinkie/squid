...
ESIStreamContext::ESIStreamContext() :
    finished(false), include(NULL), localbuffer(new ESISegment), buffer(NULL)
{
}
...
esiStreamRead(clientStreamNode *thisNode, ClientHttpRequest *http)
{
    ...
    if (context->flags.oktosend && (context->flags.finishedtemplate || context->cachedASTInUse) && !context->flags.finished) {
...
esiStreamStatus(clientStreamNode *thisNode, ClientHttpRequest *http)
{
    ...
    if (context->flags.oktosend && context->flags.finished && !(context->outbound.getRaw() && context->outbound_offset < context->outbound->len)) {
...
ESIContext::trimBlanks()
{
    ...
    while (outbound.getRaw() && outbound->next.getRaw() && !outbound->len) {
        debugs(86, 5, "ESIContext::trimBlanks: " << this << " skipping segment " << outbound.getRaw());
...
ESIContext::send()
{
    ...
    if (!(rep || (outbound.getRaw() && outbound->len && (outbound_offset <= outbound->len)))) {
...
esiProcessStream(clientStreamNode *thisNode, ClientHttpRequest *http, HttpReply *rep, StoreIOBuffer receivedData)
{
    ...
    debugs(86, 3, "esiProcessStream: Processing thisNode " << thisNode << " context " << context.getRaw() << " offset " << (int)receivedData.offset << " length " << (unsigned int)receivedData.length);
    ...
    if (receivedData.data) {
        ...
        if (receivedData.data != &context->incoming->buf[context->incoming->len]) {
            ...
            debugs(86, 5, "Copying data from " << receivedData.data << " to " << &context->incoming->buf[context->incoming->len] << " because our buffer was not used");
...
ESIContext::parserComment(const char *s)
{
    ...
    if (!strncmp(s, "esi", 3)) {
        ...
        if (!tempParser->parse("<div>", 5, 0) || !tempParser->parse(s + 3, strlen(s) - 3, 0) || !tempParser->parse("</div>", 6, 1)) {
...
{
    debugs(86, 5, "esiTryAdd: Try " << this << " adding element " << element.getRaw());
...
esiChoose::esiChoose(esiChoose const &old) :
    chosenelement(-1), otherwise(NULL), parent(NULL)
{
...
esiEnableProcessing(HttpReply *rep)
{
    ...
    if (rep->surrogate_control) {
        HttpHdrScTarget *sctusable = rep->surrogate_control->getMergedTarget(Config.Accel.surrogate_id);

        // found something targeted at us
        if (sctusable && sctusable->hasContent() && sctusable->content().pos("ESI/1.0")) {
            rv = 1;
        }

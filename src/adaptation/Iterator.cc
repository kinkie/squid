...
Adaptation::Iterator::step()
{
    ...
    if (iterations > Adaptation::Config::service_iteration_limit) {
        debugs(93, DBG_CRITICAL, "Adaptation iterations limit (" << Adaptation::Config::service_iteration_limit << ") exceeded:\n"
                                                                 << "\tPossible service loop with " << theGroup->kind << " " << theGroup->id << ", plan=" << thePlan);
...
Adaptation::Iterator::handleAdaptationError(bool final)
{
    ...
    const bool srcIntact = !theMsg->body_pipe || !theMsg->body_pipe->consumedSize();
